...
tmr.registerMock('azure-pipelines-tool-lib/tool', {
  ...
  evaluateVersions(versions, versionSpec) {
    ...
    for (let i = versions.length - 1; i >= 0; i--) {
      ...
      const satisfied: boolean = potential === 'v280.0.0';
      if (satisfied) {
        version = potential;
        break;
      }
  ...
  downloadTool(url) {
    ...
    if (url === `https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-sdk-280.0.0-${os.platform()}-${arch}.tar.gz`) {
      return 'exe_lib';
    }
