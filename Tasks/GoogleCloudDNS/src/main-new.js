...
import { checkResultAndGetData, handleError, setTaskResult } from '@shared/utils.js';
 ...
 * @param {string} managedZone The managed zone of the record
 * @param {string} name The name (address) of the record (for example www.example.com.)
 * @param {string} recordType The identifier of a supported record type.
 * @param {number} recordTtl Number of seconds that this ResourceRecordSet can be cached by resolvers.
 * @param {string} value The value of the record (like IP address if it is a record of type A)
 * @returns {Promise<*>} The change result.
 */
async function changeRecord(operation, client, project, managedZone, name, recordType, recordTtl, value) {
  const fixedName = (name.substr(-1) === '.') ? name : `${name}.`;
